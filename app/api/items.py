...
class WatchlistBody(BaseModel):
    ...
    the Reclaimer Seerr API key" (admin). Future multi-user work can populate this
...
def _resolve_watchlist_user(requested: Optional[int]):
    ...
    - ``requested`` omitted: fall back to the user that owns the admin API
      key (today: Reclaimer admin; rinseaid). That preserves single-user
...
def add_item_to_watchlist(rating_key: str, body: WatchlistBody = WatchlistBody()):
    ...
    to the user that owns the Reclaimer Seerr API key (today the admin). Seasons
